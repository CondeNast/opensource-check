...
describe("Documentation", () => {
  ...
  it("should have a License", () => {
    expect(
      fs.existsSync(`${path}/LICENSE`) || fs.existsSync(`${path}/LICENSE.md`)
    ).toEqual(true);
  });
  ...
  it("should have an Issue Template", () => {
    expect(fs.existsSync(`${path}/.github/ISSUE_TEMPLATE.md`)).toEqual(true);
  });
  it("should have a Pull Request Template", () => {
    expect(fs.existsSync(`${path}/.github/PULL_REQUEST_TEMPLATE.md`)).toEqual(
      true
    );
  });
  ...
  describe("Readme", () => {
    let pkg;
    beforeAll(() => {
      readmeContent = fs.readFileSync(`${path}/README.md`, "utf8").toString();
